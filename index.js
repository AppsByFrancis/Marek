...
const SOLANA_CONNECTION = new web3_js_1.Connection(QUICKNODE_RPC);
const FROM_KEY_PAIR = web3_js_1.Keypair.fromSecretKey(bs58_1.default.decode(process.env.PRIVATE_KEY));
 ...
 */
function generateTransactions(batchSize, dropList, fromWallet, amount) {
    const result = [];
    ...
    for (let i = 0; i < numTransactions; i++) {
        const bulkTransaction = new web3_js_1.Transaction();
 ...
 */
function isTransactionExpiredBlockheightExceededError(error) {
 ...
 */
async function executeTransactions(solanaConnection, transactionList, payer, maxRetries = 3) {
    const results = [];
        ...
        while (attempt <= maxRetries && !success) {
            try {
                ...
                if (currentBlockHeight > lastValidBlockHeight) {
                    throw new Error(`Transaction ${i + 1} has expired: current block height ${currentBlockHeight} exceeds last valid block height ${lastValidBlockHeight}`);
                }
                // Send the transaction and get the signature
                signature = await (0, web3_js_1.sendAndConfirmTransaction)(solanaConnection, transaction, [payer], {
...
async function retrieve(page, collection, network) {
    ...
    return await retry(async () => {
        const response = await axios_1.default.get("https://api.shyft.to/sol/v1/collections/get_nfts", {
            headers: {
                "x-api-key": SHYFT_KEY,
            },
            params: {
...
async function handler(collection) {
    ...
    while (page <= total) {
        ...
        nfts.push(...data.result.nfts.map((nft) => ({
            owner: nft.owner,
            address: nft.mint,
        })));
    }
